# [Maintenance] __RUN_DATE__ - Homelab
...
| **Overall Status** | 🔴 RED |
| **Last Updated** | __TS__ |
| **Source Report** | __REPORT_FILE__ |
| **Assigned To** | gitea_admin |

## Context Pack

Context pack (this run): `__CONTEXT_DIR__`
...
|-------|--------|---------|
| Nodes | 🟢 | 10/10 Ready; evidence: `__CONTEXT_DIR__/baseline-nodes.txt` |
| Node Versions | 🟢 | all nodes v1.33.6+k3s1; evidence: `__CONTEXT_DIR__/baseline-nodes.txt` |
| CNI (Cilium) | 🟢 | cilium DS 10/10, pods Running; evidence: `__CONTEXT_DIR__/system-core-kube-system.txt` |
| Kured | 🟢 | ArgoCD app `kured` Synced+Healthy; verify pods during action |

#### System Layer
| Check | Status | Details |
|-------|--------|---------|
| CoreDNS | 🟢 | `coredns` Running; evidence: `__CONTEXT_DIR__/system-core-kube-system.txt` |
| Metrics Server | 🟢 | `metrics-server` Running; evidence: `__CONTEXT_DIR__/system-core-kube-system.txt` |
| kube-vip | 🟢 | kube-vip pods Running (restarts observed but stable); evidence: `__CONTEXT_DIR__/system-core-kube-system.txt` |
| ArgoCD | 🟢 | 41/41 apps Synced+Healthy; evidence: `__CONTEXT_DIR__/baseline-argocd-apps.txt` |

#### Storage Layer
| Check | Status | Details |
|-------|--------|---------|
| Ceph Health | 🟢 | HEALTH_OK; evidence: `__CONTEXT_DIR__/storage-ceph.txt` |
| OSDs | 🟢 | 7 up / 7 in; evidence: `__CONTEXT_DIR__/storage-ceph.txt` |
| Usage | 🟢 | 35% raw used (1.2TiB/3.4TiB); evidence: `__CONTEXT_DIR__/storage-ceph.txt` |
| Pools | 🟢 | 4 pools, 177 pgs active+clean; evidence: `__CONTEXT_DIR__/storage-ceph.txt` |
| Monitors | 🟢 | 3 mons in quorum; evidence: `__CONTEXT_DIR__/storage-ceph.txt` |
| MDS | 🟢 | 1/1 up + hot standby; evidence: `__CONTEXT_DIR__/storage-ceph.txt` |

#### Platform Layer
| Check | Status | Details |
|-------|--------|---------|
| Ingress-Nginx | 🟢 | controller Running; LB 10.0.20.226; evidence: `__CONTEXT_DIR__/platform.txt` |
| Certificates | 🟢 | all certs Ready=True; evidence: `__CONTEXT_DIR__/platform.txt` |
| External Secrets | 🟢 | ExternalSecrets Ready=True, ClusterSecretStore Valid; evidence: `__CONTEXT_DIR__/platform.txt` |
| Cert-Manager | 🟢 | no pending challenges; evidence: `__CONTEXT_DIR__/platform.txt` |

#### Apps Layer
| Check | Status | Details |
|-------|--------|---------|
| All Pods | 🟢 | no CrashLoopBackOff/Error/ImagePullBackOff/Pending found; evidence: `__CONTEXT_DIR__/apps-problem-pods.txt` |
| Gitea | 🟢 | ArgoCD app `gitea` Synced+Healthy; evidence: `__CONTEXT_DIR__/baseline-argocd-apps.txt` |
| Grafana | 🟢 | ArgoCD app `grafana` Synced+Healthy; evidence: `__CONTEXT_DIR__/baseline-argocd-apps.txt` |
| Kanidm | 🟢 | ArgoCD app `kanidm` Synced+Healthy; evidence: `__CONTEXT_DIR__/baseline-argocd-apps.txt` |

### Observations

#### 🔍 Node Activity
- No critical events identified in this recon; evidence: `__CONTEXT_DIR__/baseline-events-tail-200.txt`

#### 🔍 Renovate PRs
- Open PRs: #54, #55 (both mergeable); evidence: `__CONTEXT_DIR__/gitea-open-prs.json`
- Dependency Dashboard: Issue #4; evidence: `__CONTEXT_DIR__/dependency-dashboard.md`

#### Network Evidence
- **Workstation → Cluster**: 🟢 (kubectl OK; evidence: `__CONTEXT_DIR__/kubectl-cluster-info.txt`)
- **Gitea API**: 🟢 (token validated; evidence: `__CONTEXT_DIR__/gitea-token-check.txt`)
- **NAS (10.0.40.3)**: 🟡 (NAS script warns on unauth share listing + legacy expectations; required NFS export exists: `/mnt/user/heartlib`; evidence: `__CONTEXT_DIR__/nas.json`, `__CONTEXT_DIR__/nas-showmount.txt`, `__CONTEXT_DIR__/storage-nfs-provisioner.txt`)
- **Gateway (10.0.20.1)**: 🟡 (all VLAN gateways respond on TCP 443; ICMP appears blocked -> network script reports RED; evidence: `__CONTEXT_DIR__/network.json`, `__CONTEXT_DIR__/network-gateway-tcp-443.txt`)
...
~/.config/opencode/scripts/homelab-network-check.sh --json > "__CONTEXT_DIR__/network-gate.json"

# NAS gate (exit code must be 0)
~/.config/opencode/scripts/homelab-nas-check.sh --json > "__CONTEXT_DIR__/nas-gate.json"
```
...
- [ ] **E1 P3**: [Renovate] Resolve Dependency Dashboard repo problem (GitHub token)
  ...
  - **Commands**:
    ...
    ```bash
    # Evidence
    sed -n '1,80p' "__CONTEXT_DIR__/dependency-dashboard.md"
...
- [ ] **E2 P3**: [Renovate] Decide whether to recreate blocked PR #52 (debian 13)
  ...
  - **Commands**:
    ...
    ```bash
    grep -n "pulls/52" "__CONTEXT_DIR__/dependency-dashboard.md" || true
...
| __TS__ | Recon | Issue #__ISSUE_NUM__ | Normalized issue body to contract + updated with current evidence (cluster GREEN; network gate RED due to ICMP; NAS gate YELLOW due to script expectations) | Issue body updated (no comments) | 🔴 |
