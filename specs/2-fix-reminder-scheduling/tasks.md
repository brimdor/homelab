...
- [x] T002 Update default timezone behavior for `schedule.at` parsing (apps/clawdbot/values.yaml)
- [x] T003 Default missing `schedule.tz` for `kind="at"` schedules to system timezone (apps/clawdbot/values.yaml)
...
- [x] T007 [US2] Add test case that mirrors "in X minutes" behavior via computed absolute timestamps (test/clawdbot_cron_time_test.go)

## Phase 5: Unambiguous Inspection (P2)

- [x] T008 Ensure stored schedules include timezone when omitted so UI/debug output is unambiguous (apps/clawdbot/values.yaml)
